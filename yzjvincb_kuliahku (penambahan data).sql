...
--
-- Host: 127.0.0.1
-- Generation Time: May 18, 2017 at 05:20 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cara_tugas` (`p_nama_tugas` VARCHAR(191))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_masuk_tugas` (`p_email` VARCHAR(191), `p_id_tugas` INT(10))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_matkul_list` (`p_email` VARCHAR(191))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_tugas_available` ()  BEGIN
	SELECT
	    `nama_tugas`
	FROM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_tugas_list` (`p_email` VARCHAR(191))  BEGIN
	IF EXISTS(SELECT 1 FROM users WHERE u_email = p_email) THEN
		SELECT
		   `tugas`.`nama_tugas`
		FROM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_user_statistik` (`p_email` VARCHAR(191))  BEGIN
	IF EXISTS(SELECT 1 FROM users WHERE u_email = p_email) THEN
		SELECT
		    1 as ind
		    , `users`.`u_name`
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_EN` (`p_EN_s0` TINYINT, `p_EN_t0` DATETIME) RETURNS TINYINT(4) BEGIN
...
(1, 'Pemrograman Berorientasi Objek', 3, 100, 20, 2),
(2, 'Perancangan dan Analisis Algoritma I ', 3, 100, 20, 2),
(3, 'Probabilitas dan Statistik ', 3, 100, 20, 2),
(4, 'Sistem Basis Data ', 4, 120, 27, 3),
(5, 'Sistem Operasi ', 4, 120, 27, 3),
(6, 'Teori Graf', 3, 100, 20, 2);
...
(6, 'Distribusi Normal'),
(7, 'CDM dan PDM'),
(8, 'Data, Informasi, dan Pengetahuan'),
(9, 'Normalisasi'),
(10, 'Pengetahuan dasar Linux'),
(11, 'LFS'),
(12, 'BLFS'),
